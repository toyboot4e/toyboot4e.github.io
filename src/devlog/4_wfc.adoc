= RL #1: Wave function collapse explained
...
難点は、生成に時間がかかることと、生の出力がゲームマップ向きではないことです。足場を人つながりにするなど、二段階目の加工が必要になります。
...
これらの回転 (rotation) や裏返し (flipping/reflection) も考えてみます（2x2 サイズでは、回転だけでも十分です）。重複したパタンは除きます。
...
==== Constraint = local similarity

Overlapping model では、次のような制約を課します。

TIP: 出力から展開できるパタンの集まりは、入力から展開したパタンの集まりの部分集合である。
...
逆に、 4 つのパタンを重ねれば、先ほどの出力に戻ることが分かります。これが overlapping model における制約（満たすべき条件）です。

Local similarity を満たすパタン配列をするためには、パタン同士の `compatibility` を確かめます。重なった部分のタイルが一致するなら、それらのパタンは `compatible` (overlappable) です。そうして作り上げた `AdjacencyRule` を `Solver` に渡します。

`Solver` は、互いに重なり合うことのできる `Pattern` のグリッドを作ります。このグリッドに対して、上図の右から左への加工に相当する操作 (`overlap`) を行うことにより、最終的な出力マップを得ることができます。
...
同じコア・アルゴリズム (`Solver`) を使っている以上、 `Model` がアルゴリズムのオーダを変えることはできません。ただ、 adjacency model は、 overlapping mode に比べて出力のサイズが小さくなります。なぜなら出力グリッドがパタン同士の重なり部分を含まないためです。
...
TIP: `EnablerCounts` は、 `Grid<Dictionary<Direction>>` です。
...
しかし、この表示にはさほど意味がありません。波動関数は二乗すると確率になりますが、このぼやけた表示は出現確率を表すのではなくて、もっと間接的な値（まだ配置可能なパタンの分布）を表示したものです。したがって、 WFC には波動関数に相応するものが現れず、 WFC はミスリーディングな名前だったと思います。たぶん……
...
+
　WFC がどういうものなのかよく分かります。 *おすすめです* 。 _answer set programming_ が新鮮なので、じっくり読んでみます。
