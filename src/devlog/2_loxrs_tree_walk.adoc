= Lang #1: Crafting Interpreters - 1/2
:revdate: Nov 10, 2019
...
では Rust が最強なのかと言うと、まだ検討が続いているように思います。 Rust では、 *同一変数の可変参照を 2 つ同時に作れない* 、という強い制約があります (_borrow rules_)。この制約は良い設計を促すことが多いのですが、所有権が複雑な UI プログラミングなどは困難になります。したがって、 *まだまだ研究が続く* と思います。
...
Recursive descent parser を進化させたものに https://en.wikipedia.org/wiki/Pratt_parser[Pratt parser] というものがあります。先ほど『文法を直接コードに翻訳したかのよう』と書きましたが、こちらは『Token をパーサ関数に関連付ける』そうです。 maktlad 氏が書いた https://matklad.github.io/2020/04/13/simple-but-powerful-pratt-parsing.html[記事] では、 Pratt parser を押し進め、演算子の結合順位を書くことでパーサを定義できるようにしています。
--
