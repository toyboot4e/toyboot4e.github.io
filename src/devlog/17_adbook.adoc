...
三代目 SSG は、 mdBook を目指して adbook と名付けました。

https://toyboot4e.github.io/adbook/index.html[Demo]. このブログもデモみたいなものですね。

Rust のソースは 2,000 行程度です。 html の生成は Handlebars に頼っています。

== Backstory

AsciiDoc の表現力は魅力的です。原稿は AsciiDoc で書きたいものの、生の AsciiDoc は本や単ページの生成に特化しており、ページ同士のリンクなどには向いていません。したがって `asciidoctor` の出力を利用した SSG が必要です。

AsciiDoc を使った SSG は、 https://gist.github.com/briandominick/e5754cc8438dd9503d936ef65fffbb2d[非公式リンク集] に載っています。大半の SSG には、 front matter を YAML で書くことが求められます。しかし、 AsciiDoc には _document attribute_ を設定する構文 (`:attr: value`) があるので、それを読んでくれた方が嬉しいです。したがって、 SSG を自作しています。

NOTE: ちなみに https://antora.org/[Antora] という SSG は、 AsciiDoc に特化していて良さそうなのですが、使い方が難しそうなので試していません。
...
分岐やループを使えます。再帰をするには、 partial ファイルからそれ自身を読み込みます。
...
{handlebars} を使った html の生成ができました。 `async_std` のおかげか変換が速いのが良かったと思います。ローグライク本などを作るときに使いたいと思います。

欠けた機能は、サイドバーにおける現ページのハイライトと、印刷用ページの生成です。特に後者は大変そうなので、もしかしたら実装しないかもしれません。

拡張性は無くて、 Rust 側のコードで集めたデータしか、テンプレートファイルでは使用できません。拡張性を確保するには、テーマ毎にスクリプトを用意できるようにする必要があると思います。
