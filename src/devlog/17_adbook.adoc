...
三代目 SSG を作りました。 mdBook を目指して adbook と名付けています。 https://toyboot4e.github.io/adbook/index.html[Demo]
...
{handlebars} を使った html の生成ができました。 `async_std` のおかげか変換が速いのが良かったと思います。今後、ローグライク本などを書くときなどに、使っていきたいと思います。

欠けた機能は、サイドバーにおける現ページのハイライトと、印刷用ページの生成です。特に後者は大変そうなので、もしかしたら実装しないかもしれません。

テーマの拡張性は無くて、テンプレートファイルの中では、 Rust 側のコードから注入されるデータしか使用できません。拡張性を確保するには、テーマ毎にスクリプトを用意できるようにする必要があると思います。
