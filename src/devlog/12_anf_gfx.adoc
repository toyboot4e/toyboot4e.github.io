...
* GPU が効率的に使えるメモリがあり、 GPU が使用するデータ (頂点データや texture) はそこに入れます。 CPU 側は、 GPU 側メモリへのデータの _upload_ (send/copy) 後は、 CPU 側のメモリを開放しても構いません。普通は開放します。
...
https://github.com/floooh/sokol/blob/master/sokol_gfx.h[sokol_gfx] は分かりやすそうです。非常によくコメントされており、多数のデータを束ねる型もあります (`sg_bindings`, `sg_pipeline`) footnote:[Sokol で分かりにくそうなのは、 `#ifdef` を使っている部分です。 `#ifdef` を読み飛ばすと、かなり分かりやすく見えました] 。一方、 FNA3D は不親切で、中々把握できません。

そこで、 *インクリメンタルな開発* に振り切ることにしました。僕が知らない機能、僕が使わない機能は実装されませんし、 https://www.google.com/search?q=WIP[WIP] のコードもありません。『見れば分かる』コードになったと思います。
...
`DrwaContext` はオブジェクト指向の API で、他のフレームワークと比べても意外と良い出来になりそうです。

目下の悩みは、 FNA3D の自動ビルド & 出力への同封です。 https://github.com/alexcrichton/cmake-rs[cmake-rs] でささっと行けるかと思ったのですが、苦戦しています。
