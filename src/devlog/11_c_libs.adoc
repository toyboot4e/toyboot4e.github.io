= Note #3: C libraries and modern C-like langs
:revdate: Aug 2, 2020
...
同作者は https://rxi.itch.io/tilekit[Tilekit] のクローン footnote:[Mac でも動くのが、 Tilekit との違いの 1 つです] を Upaya で制作したようです (未公開) 。 Electron と比べた場合のメリットは、 *開発環境のシンプルさ* と *アプリの動作速度* だと思います。ただ、 game-in editor が欲しいときはゲームの開発環境を使うと思うので、僕が直接真似するのは ImGUI を多用する姿勢だけになるかもしれません。

NOTE: Upaya では、マルチウィンドウや組み込みターミナルができないと思います。日本語入力もできないかも……？　ただ、大きなツールがヘヴィ過ぎてオーバーキルとなる場面もあるので、そのときは Upaya が輝くと思います。
...
色々書きましたが、僕は C をあまり使ったことがないので分かりません。

== Wrapping up

C ライブラリの良さを確認しました。 Zig は C との親和性が非常に高く、 Zig + C ライブラリの組み合わせは理想的らしいです。
