...
もしも木構造などを作りたければ、 visitor 側でやってもらいます。そのときは、 html 要素のツリーのような、 *パース時とは違った文脈で node を作る* と思います。やはり、任意の parse 要素をアップキャストするような `enum` は要らないと考えました。
...
serde を真似ることで、 outline パーサを綺麗に書くことができました。特に access が活躍しましたし、他の場所でも応用が利くと思います。
