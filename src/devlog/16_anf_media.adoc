...
C は Rust からでも普通に呼べます。ただ borrow rules はガン無視する方向へ心が傾きました。所有権を考慮して wrapper を作るよりも、所有権を無視してポインタをコピーします。
...
+
タイルセット (Tiled の `.tsx` ファイル) のタイルに _property_ を設定することで、当たり判定を設定できる気がします。たとえば、壁タイルは『通行不可』とします。
+
どの道、実行時には `blocks` 変数を作ることになるため、 Tiled の `collision` レイヤから当たり判定を設定できるのは悪くない機能です。
...
元々 Tiled マップを手動で表示していたので、このような追加は簡単でした。マップ・ FoV/FoW 表示をクロージャを使ったコードにリアァクタリングして、今回はおしまいです。
