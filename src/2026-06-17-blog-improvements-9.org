...
前回の投稿でサムネイルの表示し、重くなったブログを高速化しました。概ね AI エージェントがやってくれました。
...
トップページのサムネイルが重かったようです。 webp に変換するだけで 50% 以下に容量を削減できました。
...
テンプレート処理中で、特定の要素に応じて =<link>= タグを設定するように変更しました。たとえば =$\LaTeX$= と書けば、 =org-mode= はまず以下の math fragment に展開します:
