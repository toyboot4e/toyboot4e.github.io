...
[[https://toyboot4e.github.io/plover-drills][Plover Drills for Lapwing Theory]] ← のリンクをクリックしてご覧ください。
...
入力値が変化した時に状態を初期化したいとします。この場合は、入力値を元にコンポーネントの =key= を設定すると、 =key= の変更時に新たなコンポーネントが作成されます:
