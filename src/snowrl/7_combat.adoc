...
次回はイカちゃん 🦑 がタコ殴りに遭う絵を撮りたいです！

=== No more `mod.rs`

ついに `mod.rs` を廃止して、 `module_name.rs` を使い始めました。始めは抵抗感があったのですが、すぐ良さに気づきました。 *コードを読む間、ディレクトリ移動の回数が減りました* 。

当時の人は、よくこのクールな方式を思いつきましたね。 Rust で一番好きなところは、式指向とモジュールシステムです。 2 つだった。
...
とはいえ、この organization には誰でも気軽に参加できるようです。誰でも rg3d 開発グループの一員となってくれて良い、そんなメッセージを感じて心温まりますね。それが https://rg3d.rs/[mrDIMAS] だ！　彼をサポートしてください。アクティブユーザが数人しかいないエンジンですので、参加者・ユーザがどんどん増えてくれたらと思います。

.Maaaaaaaaaaaaaaaac
[sidebar]
--
ブラウザでも動くようになった `rg3d` ですが、 *macOS 上では実質動きません* 。 [.line-through]#そう、僕は `rg3d` エアプレイなのです# mac 上の OpenGL は、パフォーマンスに問題があるようです。現在、 XCode で OpenGL のプロファイルを取る方法を調べていますが、果たして治るかどうか……。修正できない目も大きいです。
...
今回から、キャラを一般 `Node` として描きます (retained-mode) 。キャラ画像と体力ゲージは、キャラ `Node` からの相対位置で描かれます (transform) 。また、 `Node` のプールを 1 つにして、 `Layer` を `Node` のフィールドとしました。
...
冴えないですが、これで演出をハードコーディングします。次回は、スクリプト部分 (キャラの会話など) に取り組んで行く予定です。ホットリロードができなかったので、 GameLisp か自前の AST を試します。
...
ちなみに、一番好きなバンドは Obscura です。ライブも行ったのですよね
