...
ビルドに Emacs を使うのをやめ、 TypeScript 製の org パーサである [[https://github.com/rasendubi/uniorg][Uniorg]] に移行しました。 Emacs の起動を待つ必要が無くなったため、ビルドを高速化できました。
...
#+CAPTION: Block $KaTeX$, lowercase mathbb
$$
\mathbb{F} = m \ddot{\mathbb{x}}
$$
