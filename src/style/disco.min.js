...
`;function F(e,o,l){let a=e.createShader(o);return a?(e.shaderSource(a,l),e.compileShader(a),e.getShaderParameter(a,e.COMPILE_STATUS)?a:(console.warn("disco: shader compile failed:",e.getShaderInfoLog(a)),e.deleteShader(a),null)):null}function U(e){let o=F(e,e.VERTEX_SHADER,ae),l=F(e,e.FRAGMENT_SHADER,ie);if(!o||!l)return null;let a=e.createProgram();return a?(e.attachShader(a,o),e.attachShader(a,l),e.linkProgram(a),e.getProgramParameter(a,e.LINK_STATUS)?a:(console.warn("disco: program link failed:",e.getProgramInfoLog(a)),null)):null}var H="toybeam-disco";function re(){try{return localStorage.getItem(H)!=="off"}catch(e){return!0}}function ne(e){try{let o=e.getExtension("WEBGL_debug_renderer_info");if(!o)return!1;let l=String(e.getParameter(o.UNMASKED_RENDERER_WEBGL)||"").toLowerCase();return/swiftshader|llvmpipe|softpipe|software|basic render|microsoft basic|mesa offscreen/.test(l)}catch(o){return!1}}function b(e){return window.matchMedia?window.matchMedia(e):null}function W(e,o){e&&(e.addEventListener?e.addEventListener("change",o):e.addListener(o))}function se(e){try{return new URL(e).origin===location.origin}catch(o){return!1}}function le(){try{let e=sessionStorage.getItem("toybeam-disco-t0");if(e)return parseFloat(e);let o=Date.now();return sessionStorage.setItem("toybeam-disco-t0",String(o)),o}catch(e){return Date.now()}}function ce(){let e=document.documentElement.getAttribute("data-theme");if(e==="dark")return!0;if(e==="light")return!1;let o=b("(prefers-color-scheme: dark)");return!!(o&&o.matches)}function Y(){let e=document.getElementById("disco-canvas");if(!e)return;function o(){document.documentElement.classList.remove("disco-on","disco-static"),e.remove();let i=document.querySelector(".disco-bg-light");i&&i.remove();let r=document.getElementById("disco-toggle");r&&r.remove()}let a=location.pathname==="/"||/(^|\/)index\.html$/.test(location.pathname)?"1":"0.55",K=!!document.referrer&&se(document.referrer),y={alpha:!0,antialias:!1,depth:!1,stencil:!1,premultipliedAlpha:!1,powerPreference:"low-power"},t=e.getContext("webgl",y)||e.getContext("experimental-webgl",y);if(!t){o();return}if(ne(t)){o();return}let n=U(t);if(!n){o();return}let q=t.createBuffer();t.bindBuffer(t.ARRAY_BUFFER,q),t.bufferData(t.ARRAY_BUFFER,new Float32Array([-1,-1,3,-1,-1,3]),t.STATIC_DRAW);let P=t.getAttribLocation(n,"a_pos");t.enableVertexAttribArray(P),t.vertexAttribPointer(P,2,t.FLOAT,!1,0,0),t.useProgram(n);let z=t.getUniformLocation(n,"u_res"),X=t.getUniformLocation(n,"u_time"),V=t.getUniformLocation(n,"u_ballOnly"),j=t.getUniformLocation(n,"u_light"),S=b("(pointer: coarse)"),k=!!(S&&S.matches),Q=k?1:1.5,$=k?.7:1;function J(){let i=Math.min(window.devicePixelRatio||1,Q)*$*E,r=Math.max(1,Math.round(e.clientWidth*i)),A=Math.max(1,Math.round(e.clientHeight*i));(e.width!==r||e.height!==A)&&(e.width=r,e.height=A),t.viewport(0,0,r,A)}let L=b("(prefers-reduced-motion: reduce)"),Z=le(),d=0,h=!1,g=!1,m=re(),_=!1,s=0,p=0,w=1e3/30,E=1,I=!1,f=0,O=0,v=0,x=0;function T(){I=!0,u(),o()}function ee(){E>.34?E*=.6:T()}function B(i,r){J(),t.uniform2f(z,e.width,e.height),t.uniform1f(X,i),t.uniform1f(V,r?1:0),t.uniform1f(j,_?1:0),t.drawArrays(t.TRIANGLES,0,3),g||(g=!0,e.style.opacity=a)}function C(i){if(!h||(d=window.requestAnimationFrame(C),f&&i-f<w-1))return;let r=f?i-f:w;if(f=i,++O>5){if(r>150){if(++x>=3){T();return}}else x=0;if(r>w*2){if(++v>=10){v=0,ee();return}}else v=0}B((Date.now()-Z)/1e3,!1)}function te(){return!!(L&&L.matches)}let R=0;function D(){document.documentElement.classList.toggle("disco-static",!_),!R&&(R=window.setTimeout(()=>{R=0,B(9,!0)},0))}function oe(){if(!h){if(te()){D();return}document.documentElement.classList.remove("disco-static"),h=!0,f=0,O=0,v=0,d=window.requestAnimationFrame(C)}}function u(){h=!1,d&&window.cancelAnimationFrame(d),d=0}function M(){document.documentElement.classList.add("disco-animating"),p&&clearTimeout(p),p=window.setTimeout(()=>{document.documentElement.classList.remove("disco-animating"),p=0},900)}function c(){if(!I)if(_=!ce(),m)document.documentElement.classList.add("disco-on"),s&&(clearTimeout(s),s=0,e.style.opacity=a),document.hidden?(u(),e.style.opacity="0",g=!1):oe();else{document.documentElement.classList.remove("disco-on","disco-static"),e.style.opacity="0";let i=()=>{u(),g=!1,s=0};!document.hidden&&h?(s&&clearTimeout(s),s=window.setTimeout(i,850)):(s&&clearTimeout(s),i())}}let N=document.getElementById("disco-toggle");function G(){N&&N.setAttribute("aria-pressed",m?"true":"false")}G(),window.toggleDisco=function(){m=!m;try{localStorage.setItem(H,m?"on":"off")}catch(i){}G(),M(),c()},new MutationObserver(c).observe(document.documentElement,{attributes:!0,attributeFilter:["data-theme"]}),W(b("(prefers-color-scheme: dark)"),c),W(L,()=>{u(),c()}),document.addEventListener("visibilitychange",c),window.addEventListener("resize",()=>{!h&&document.documentElement.classList.contains("disco-on")&&D()}),e.addEventListener("webglcontextlost",i=>{i.preventDefault(),u()},!1),e.addEventListener("webglcontextrestored",()=>{n=U(t),n&&(t.useProgram(n),c())},!1),K||M(),c()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Y):Y();})();
