...
----
<1> `Pipeline` には、シェーダ、 texture, 頂点や index のデータ形式が入っています。
<2> `Binding` には index/vertex buffer が入っています。
...
Zig の初期値は https://github.com/ziglang/zig/issues/485[センスが良くて] いいですね。型名を `.` で省略できるのが良いです。
...
実は `if let Some(..) = ..` 相当のコードも `if variable |v|` と書けて非常に良いです。
...
{sokol} は header-only のライブラリ集です。 `sokol_app.h` でウィンドウを出して、 `sokol_gfx.h` で画像を表示します。 `sokol_app.h` は *ゲームパッドのサポートなどがまだまだ* みたいなので、機能不足かもしれません。
...
{sokol} の API は、 WebGPU などを参考に作られたようです。 Pipeline や Pass といったオブジェクトがあります。 API の背後にはグローバルオブジェクトがあり、 _resource_ はそこに保存されます。ユーザは保存された resource (buffer, image pass, pipeline, shader) への ID を持つことができます。
