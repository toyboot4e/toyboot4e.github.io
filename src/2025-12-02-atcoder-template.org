...
[[https://hackage.haskell.org/package/base-4.21.0.0/docs/Data-Ix.html#v:index][=Data.Ix.index=]] には [[https://hackage.haskell.org/package/base-4.21.0.0/docs/GHC-Stack.html#t:HasCallStack][=HasCallStack=]] 制約が無いため、エラー時にどこでエラーが起きたか分かりません。エラー表示は以下のようになります:

#+BEGIN_SRC txt
a-exe: Error in array index
#+END_SRC

より詳細なエラー表示をしてくれる =index= 関数が以下です:
...
[[https://hackage.haskell.org/package/vector][=vector=]] パッケージは 1 次元配列を提供します。 =vector= をラップすると、 =array= 似の多次元配列の API を提供できます。
